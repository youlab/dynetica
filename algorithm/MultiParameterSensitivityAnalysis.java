...
public class MultiParameterSensitivityAnalysis implements Runnable {
    ...
    ArrayList xVariables; // variables to be changed, one by one. Here either
                          // parameters or substances can be used.
    // dynetica.util.DoubleList minValues; // stores the minimum values of the
    // specified xVariables
    // dynetica.util.DoubleList maxValues; // stores the maximum values of
    // xVariables
    // dynetica.util.DoubleList numPoints; // sotres the number of points to go
    // through for each variable
    int numPoints = 20; // number of different values (within the specified
    ...
    double[][][][] timeCourses; // stores the time courses of objective
                                // functions resulting from the different
                                // variable values
    ...
    public void run() {
        if (numPoints <= 0 || xVariables == null || metricList == null) {
            System.out.println("Error in parameters. NumSim = " + numPoints
                    + " Variables = " + xVariables + " metricList = "
                    + metricList);
            return;
        }
        ...
        for (int j = 0; j < xVariables.size(); j++) { // first layer scanning
            ...
            for (int i = 0; i <= numPoints; i++) { // 2nd layer scanning through
                ...
                if (!logScale) {
                    currentValue = i * (currentMax - currentMin) / numPoints
                            + currentMin;
                } else {
                    currentValue = Math.exp(i
                            * (Math.log(currentMax) - Math.log(currentMin))
                            / numPoints + Math.log(currentMin));
                }
                ...
                //
                EntityVariable sysVariable = (EntityVariable) (system
                        .getEntity(variable.getName()));
                ...
                //
                // Reset the value or initial value of the correponding system
                // variable (parameter or substance)
                //
                if (sysVariable instanceof Parameter)
        ...
        if (plotOnComplete) {
            // dynetica.gui.SensitivityFigureWindow s = new
            // dynetica.gui.SensitivityFigureWindow(this);
            // s.show();
        }
