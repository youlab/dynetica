...
public class ExpressionVariable extends Substance {
     ...
     * 
     * @param expression
     *            New value of property expression.
     */
    ...
    public void setExpression(String expStr) {
        if (expStr != null)
            try {
                this.expression = ExpressionParser.parse(this.getSystem(),
                        expStr);
...
//            System.out.println(exps);
            for (GeneralExpression exp : exps) {
                if (exp instanceof Substance) {
                    Substance s = (Substance) (exp);
    ...
    // @Override
    // Lingchong You Note: As the expression should be properly updated in
    // simualtions using update(). There should be no need to override
    ...
    //
    // Added 05/04/2006, by Lingchong You to allow correct updating of values in
    ...
    //added by LY (6/2016) to allow clearing of "memory" by some calculations.
    @Override
    public void reset(){
       super.reset();
       expression.reset();
    ...
    public String getCompleteInfo() {
        return (getFullName() + " {" + NEWLINE + " Expression { "
                + expression.toString() + "}"
                + NEWLINE
                +
    ...
    public void setProperty(String propertyName, String propertyValue)
        ...
        if (propertyName.compareToIgnoreCase("Expression") == 0)
            ...
            throw new UnknownPropertyException(
                    " Unknown property for ExpressionVariable " + propertyName);
    }
