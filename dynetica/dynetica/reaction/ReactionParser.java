        ...
        while (strTokens.hasMoreTokens()) {
            ...
            if (Character.isLetter(element.charAt(0))) {
                ...
                if (tempName.length() > 1
                        && (tempName.charAt(0) == 'e' || tempName.charAt(0) == 'E')
                        && tempName.charAt(1) == '-') {
                    tempIndex = 2;
                    while (Character.isDigit(tempName.charAt(tempIndex))
                            || element.charAt(tempIndex) == '.')
                        tempIndex++;
                    powerNumber = Double.parseDouble(tempName.substring(2,
                            tempIndex));
                    ...
                    substanceName = tempName.substring(tempIndex).trim();
                        ...
                        && (tempName.charAt(0) == 'e' || tempName.charAt(0) == 'E')
                        && Character.isDigit(tempName.charAt(1))) {
                    tempIndex = 1;
                    while (Character.isDigit(tempName.charAt(tempIndex))
                            || tempName.charAt(tempIndex) == '.')
                        tempIndex++;
                    powerNumber = Double.parseDouble(tempName.substring(1,
                            tempIndex));
                    powerNumber = Math.pow(10.0, powerNumber);
                    coefficient = coefficient * powerNumber;
                    substanceName = tempName.substring(tempIndex).trim();
                    ...
                    if (!strTokens.hasMoreTokens())
                        ...
                        if (Character.isDigit(subSt_next.charAt(0))) {
                            tempIndex = 0;
                            while (Character.isDigit(subSt_next
                                    .charAt(tempIndex))
                                    || subSt_next.charAt(tempIndex) == '.')
                                tempIndex++;
                            powerNumber = Double.parseDouble(subSt_next
                                    .substring(0, tempIndex));
                            powerNumber = Math.pow(10.0, powerNumber);
                            coefficient = coefficient * powerNumber;
                            substanceName = subSt_next.substring(tempIndex)
                                    .trim();
                        } else {
                            e_asSubst = true;
                            substanceName = "E";
