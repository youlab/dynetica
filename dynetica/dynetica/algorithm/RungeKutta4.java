...
public class RungeKutta4 extends Algorithm {
    ...
    public RungeKutta4(ReactiveSystem system, double maxStep,
            double samplingStep, int iterations) {
        super(system);
        setMaxStep(maxStep);
    ...
    public void update() {
        ...
        while (t < samplingStep) {
            ...
            for (int i = 0; i < numberOfSubstances; i++) {
                ...
                s[i].setValue(y1[i] + (k1[i] + 2 * k2[i] + 2 * k3[i] + k4[i])
                        / 6.0);
            }
    ...
    public javax.swing.JPanel editor() {
        return new dynetica.gui.algorithms.RK4Editor(this);
    }
} // RungeKutta4
