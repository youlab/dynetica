...
public class MichaelisMentenReactionEditor extends javax.swing.JPanel {
    ...
    public MichaelisMentenReactionEditor(dynetica.reaction.MichaelisMentenReaction r) {
        reaction = r;
        initComponents ();
        updateListModels();
    }
    ...
    private void updateListModels() {
        ...
        if (! substanceListModel.isEmpty()) sList.setSelectedIndex(0);
        ...
        if (! parameterListModel.isEmpty()) pList.setSelectedIndex(0);        
    }
    ...
    private void initComponents() {
        ...
        jLabel1.setText("<html> <i> Michaelis-Menten Reaction </i>" + reaction.getName());
  ...
  private Parameter getCurrentParameter() {
      ...
      //If condition added by Kanishk Asthana on 23 July 2013: See pListvaluechanged for and explaination
      if(index!=-1)
      {   
  ...
  private void pListValueChanged(javax.swing.event.ListSelectionEvent evt) {//GEN-FIRST:event_pListValueChanged
      ...
      //was executed all elements were removed from the plist. Since this removal of elements triggered  
      //a pListvaluechanged event; the event led to this method being executed. This method would then try to get
  ...
  private void setSubstrate() {
      ...
      if (substrateName.length() > 0 && substrateName.compareTo(oldSubstrateName) != 0) {
          if (reaction.getSystem().contains(substrateName))
              reaction.setSubstrate(reaction.getSystem().getSubstance(substrateName));
          else {
              dynetica.entity.Substance s = new dynetica.entity.Substance(substrateName, reaction.getSystem());
  ...
  private void setProduct() {
      ...
      if (productName.length() > 0 && productName.compareTo(oldProductName) != 0) {
          if (reaction.getSystem().contains(productName))
              reaction.setProduct(reaction.getSystem().getSubstance(productName));
          else {
              dynetica.entity.Substance s = new dynetica.entity.Substance(productName, reaction.getSystem());
  ...
  private void setEnzyme() {
      ...
      if (enzymeName.length() > 0 && enzymeName.compareTo(oldEnzymeName) != 0 ) {
          if (reaction.getSystem().contains(enzymeName))
              reaction.setEnzyme(reaction.getSystem().getSubstance(enzymeName));
          else {
              dynetica.entity.Substance s = new dynetica.entity.Substance(enzymeName, reaction.getSystem());
