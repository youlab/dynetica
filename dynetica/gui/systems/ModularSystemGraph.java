...
public class ModularSystemGraph extends javax.swing.JPanel 
    implements DrawingConstants, Printable{
   ...
   // Holds the coordinates of the user's last mousePressed event.
    ...
    javax.swing.event.EventListenerList listenerList = new javax.swing.event.EventListenerList();//empty list
    ...
    public void fireSizeChange() {
        EventListener[] listeners = listenerList.getListeners(SizeChangeListener.class);
    ...
    protected void addSystemStructureChangeListener(SystemStructureChangeListener l) {
        system.addSystemStructureChangeListener(l);
    }
    
    /**
     * Reads coordinates from File in.
     * the name of in should be the name of the input file for the system plus ".graph" 
     * This method is called when a system is opened.
     * 
    ...
    /**
     * Saves the graph into the output file. The name of out should be the output file for the
     * system plus ".graph". This method is called everytime the system is saved.
     */
    ...
    private void storeCurrentCoordinates(){
        ...
        for (int i = 0; i < substanceNodes.length; i ++ ) {
               ...
               coordinateMap.put(node, new Point((int) node.getX(), (int) node.getY()));
        }
   ...
   //
    ...
    public void trim() {
        ...
        for (int i = 0; i < substanceNodes.length; i ++ ) {
               node = (AbstractNode) substanceNodes[i];
               if (node.getX() > maxX) maxX = node.getX()+ layout.margin;
               if (node.getY() > maxY) maxY = node.getY()+ layout.margin;
        }
    ...
    private AbstractNode getCurrentNode(MouseEvent e) { 
        ...
        for (int i = 0; i < substanceNodes.length; i++) {
            if (((AbstractNode) (substanceNodes[i])).contains((double) e.getX(), (double) e.getY())) {
                node = (AbstractNode) (substanceNodes[i]);
        ...
        for (int i = 0; i < reactionNodes.length; i++) {
            if (((AbstractNode) (reactionNodes[i])).contains((double) e.getX(), (double) e.getY())) {
               node= (AbstractNode) (reactionNodes[i]);
                node.setSelected(true);
                return node;
            }
        ...
        for (int i=0; i < expressionNodes.length; i++){
            if (((AbstractNode) (expressionNodes[i])).contains((double) e.getX(), (double) e.getY())) {
               node= (AbstractNode) expressionNodes[i];
                 node.setSelected(true);
                return node;
            }
        ...
        for (int i=0; i < moduleNodes.length; i++){
            if (((AbstractNode) (moduleNodes[i])).contains((double) e.getX(), (double) e.getY())) {
               node= (AbstractNode) moduleNodes[i];
                 node.setSelected(true);
                return node;
            }
   ...
   //Note to self(Kanishk Asthana): This function has complicated logic: write a description
    protected void reset() {
       if (substanceNodes != null)
        ...
        int reactionNumber = system.getProgressiveReactions().size() + system.getEquilibratedReactions().size();
      ...
      {                
        ...
        expressionNodesForModules= expressionNodeMapForModules.values().toArray();
        
        dynetica.reaction.Reaction r;
           
        for (int i = 0; i < system.getProgressiveReactions().size(); i++) {
            r = (dynetica.reaction.Reaction) (system.getProgressiveReactions().get(i));
        ...
        for (int i = 0; i < system.getEquilibratedReactions().size(); i++) {
            r = (dynetica.reaction.Reaction) (system.getEquilibratedReactions().get(i));
           if(!(r.getSystem()instanceof AbstractModule))   
            reactionNodeMap.put(r, r.getNode());
           else if(((AbstractModule)r.getSystem()).isPopedOut())
            reactionNodeMap.put(r, r.getNode());
        ...
        for (int i = 0 ; i<((ModularSystem) system).getModules().size(); i++)
          {
           m = (dynetica.system.AbstractModule)((ModularSystem) system).getModules().get(i);
           if(!(m.isPopedOut()))
    ...
    protected void setLocation(AbstractNode node, double x, double y) {
        if (x > getWidth()) x = getWidth()-layout.margin;
        else if (x < layout.margin) x = layout.margin;        
        if (y > getHeight()) y = getHeight() -layout.margin;
        else if (y < layout.margin) y = layout.margin;
        node.setLocation(x, y);
    }
//Kanishk Asthana
    public void drawGraphForModularSystem(Graphics2D g2, int w, int h, int offsetX, int offsetY) {
        ...
        for (int i = 0; i < expressionNodes.length; i++) {
            ...
            if (exp.getX()<layout.margin) {
                    double theta = ((double) i / expressionNodes.length) * 2.0 * pi + pi/2.718 ;
        ...
        for (int i = 0; i < reactionNodes.length; i++) {
          AbstractNode  rn = (AbstractNode) reactionNodes[i];
            if (rn.getX()<layout.margin) {
                    double theta = ((double) i / reactionNodes.length) * 2.0 * pi + pi/2.718 ;
        ...
        for (int i = 0; i < moduleNodes.length; i++) {
            ...
            if (md.getX()<layout.margin) {
                    double theta = ((double) i / moduleNodes.length) * 2.0 * pi + pi/2.718 ;
    ...
    protected RNode getReactionNode(Reaction r) {
        if (reactionNodeMap.containsKey(r))
            ...
            rn.setLocation (Math.random() * getWidth() * 0.8, Math.random() * getHeight() * 0.8);
...
//Kanishk Asthana        
       protected void drawLinesToModularSystem(Graphics2D g) {
        ...
        for (int i = 0; i < reactionNodes.length; i ++) {
            ...
            //Something is wrong here, I'm pretty sure! this may cause a bug later! FIGURE IT OUT!
            ...
            if(!(s.getSystem() instanceof AbstractModule))
            {
                ...
                if (sn != null) {
                    ...
                    new ArrowedLine(x2, y2, x1, y1, layout.arrowSize, 0.4, drawArrows, false).draw(g);
                }
            }
            else if(((AbstractModule)s.getSystem()).isPopedOut()){
               AbstractNode sn = ((Substance) reactants.get(j)).getNode();
            ...
            else //This may be problematic when extending the code: be carefull!
            {
              if(((AbstractModule)s.getSystem()).getNode()!= null)
             {
              double x2= ((AbstractModule) s.getSystem()).getNode().getCenterX();
              double y2= ((AbstractModule) s.getSystem()).getNode().getCenterY();
              new ArrowedLine(x2, y2, x1, y1, layout.arrowSize, 0.4, drawArrows, false).draw(g);
             }             
            }
          } 
            ...
            if(!(s.getSystem() instanceof AbstractModule))
            {
                ...
                if (sn != null) {
                    double x2 = sn.getCenterX();
                    double y2 = sn.getCenterY();
                    new ArrowedLine(x1, y1, x2, y2, layout.arrowSize, 0.4, drawArrows, false).draw(g);
                }
             }
            else if(((AbstractModule)s.getSystem()).isPopedOut()){
               AbstractNode sn  = ((Substance) products.get(j)).getNode();
        ...
        //
        // draw lines connecting an ExpressionNode to its dependent substances and expressions.
        // 
        ...
        for (int i = 0; i < expressionNodes.length; i++) {
            ...
            if (en != null) {
                ...
                java.util.List substances = ((ExpressionVariable)(en.getEntity())).getSubstances();
                for (int j=0; j <substances.size(); j++) {
                    Substance s= (Substance)substances.get(j);
             //       System.out.println(substances.get(j));
                    if(!(s.getSystem() instanceof AbstractModule))
                   {
                    AbstractNode sn = ((Substance) (substances.get(j))).getNode();
                    if (sn!= null){
                        double x2 = sn.getCenterX();
                        double y2 = sn.getCenterY();
       ...
       protected void drawLinesFromModularSystem(Graphics2D g)
       {
         ...
         for (int i = 0; i < reactionNodesForModules.length; i ++) {
            ...
            AbstractModule superSystem=(AbstractModule)rn.getReaction().getSystem(); 
            ...
            for (int j = 0; j < reactants.size(); j++) {
                Substance s= (Substance) reactants.get(j);
                AbstractSystem substanceSystem=system.get(s.getName()).getSystem();
        ...
        for (int i = 0; i < expressionNodesForModules.length; i++) {
         ...
         if(en!=null){
         AbstractModule superSystem= (AbstractModule) en.getEntity().getSystem();    
         MNode md=(MNode)superSystem.getNode();
         ...
         for (int j=0; j <substances.size(); j++) {
          ...
          if(!(substanceSystem instanceof AbstractModule))
            ...
            else if(!substanceSystem.equals(en.getEntity().getSystem())
                     && ((AbstractModule)s.getSystem()).isPopedOut())
            {
                    AbstractNode sn = ((Substance) (substances.get(j))).getNode();
                    if (sn!= null){
    ...
    private void initComponents() {
        ...
        lineWidthIncreaseItem.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
        ...
        lineWidthDecreaseItem.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
        ...
        increaseArrowItem.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
        ...
        decreaseArrowItem.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
    ...
    private void lineWidthDecreaseItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_lineWidthDecreaseItemActionPerformed
        changeLineWidth(0.6f);
        relativeLineWidth *= 0.6f;
        repaint();
    }//GEN-LAST:event_lineWidthDecreaseItemActionPerformed

    private void lineWidthIncreaseItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_lineWidthIncreaseItemActionPerformed
  ...
  private void shrinkGraphActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_shrinkGraphActionPerformed
      setPreferredSize(new Dimension((int) (getWidth() * 0.85), (int) (getHeight() * 0.85)));
     fireSizeChange();
      repaint();
  ...
  private void growGraphActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_growGraphActionPerformed
      setPreferredSize(new Dimension((int) (getWidth() * 1.15), (int) (getHeight() * 1.15)));
      repaint();
      fireSizeChange();
  ...
  private void hideNameBoxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_hideNameBoxActionPerformed
    ...
    private void popOutItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_popOutItemActionPerformed
       ...
       else System.out.println("Not Module:This should not happen!");
 ...
 public void changeNodeSize(double ratio) {
    ...
    public class GraphMouseListener implements MouseListener {
            ...
            public void mousePressed(java.awt.event.MouseEvent e) {
                if (currentNode!=null) currentNode.setSelected(false);
            ...
            private void showPopup(java.awt.event.MouseEvent e) {
                ...
                if (e.isPopupTrigger()) {
                    ...
                    else popOutItem.setVisible(false);
    ...
    protected void addGesture(java.awt.event.MouseEvent e){
      if(currentNode!=null && !(currentNode.getEntity() instanceof AbstractModule)) {
                for (int i=0; i < moduleNodes.length; i++)
                 if (((AbstractNode) (moduleNodes[i])).contains((double) e.getX(), (double) e.getY()))
                  {
                      AbstractModule mod=(AbstractModule)((AbstractNode)moduleNodes[i]).getEntity();
                      Entity entityToTransfer=currentNode.getEntity();
                   
                        entityToTransfer.setSystem(mod);
                       ((ModularSystem)system).removeEntityFromTreeModel(entityToTransfer);
                        mod.insertEntityIntoTreeModel(entityToTransfer);
                        //Setting a negative value of X repositions the entity automatically
    ...
    public int print(Graphics g, PageFormat pf, int pi) throws PrinterException {
        ...
        drawGraphForModularSystem((Graphics2D) g, w, h,(int) pf.getImageableX(), (int) pf.getImageableY());
        restoreCoordinates();
