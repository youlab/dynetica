...
public class GenomeEditor extends javax.swing.JPanel {
    ...
    public GenomeEditor(Genome genome) {
        ...
        if (elements.getSize() > 0) elementList.setSelectedIndex(0);
        this.addSystemStructureChangeListener(new SystemStructureChangeListener() {
            public void systemStructureChanged(SystemStructureChangeEvent e) {
                ...
                if (elements.getSize() > 0) {
                    elementList.setSelectedIndex(Math.min(elementList.getSelectedIndex(), elements.getSize() - 1));
                }
           }
        });
    }
    ...
    private void setupComboBoxModel() {
        String [] types = {"Gene", "GenomeEnd", "Promoter", 
        "RNASplicingSite", "Spacer", "Terminator"};
    ...
    private void setupListModel() {
        if (! elements.isEmpty() ) elements.removeAllElements();
 //       System.out.println(genome.getElements().size());
        for (int i = 0; i < genome.getElements().size(); i ++) {
            elements.addElement(((GeneticElement) (genome.getElements().get(i))).getName());
        }       
    }
    ...
    private void initComponents() {
        ...
        elementList.addListSelectionListener(new javax.swing.event.ListSelectionListener() {
  ...
  private void elementListValueChanged(javax.swing.event.ListSelectionEvent evt) {//GEN-FIRST:event_elementListValueChanged
     setSelectedElement();
  ...
  private void addBoxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addBoxActionPerformed
      try {
        GeneticElement element = (GeneticElement) (Class.forName("dynetica.entity." + addBox.getSelectedItem()).newInstance());
...
//        elements.addElement(newName);
  ...
  //
    ...
    if (! elements.isEmpty()) {
        ind = Math.min(elements.size() - 1, ind); // set the new selected item
  ...
  private void renameElement() {
      ...
      GeneticElement ge = (GeneticElement) (genome.get(((String) elements.elementAt(ind))));
  ...
  private void setSelectedElement() {
    ...
    javax.swing.JPanel elementEditor =((GeneticElement) (genome.get(elementName))).editor(); 
