...
public class EquationsMATLAB extends javax.swing.JPanel {
    ...
    private String getEquations() {
        ...
        double t_final = system.getAlgorithm().getIterations() * system.getAlgorithm().getSamplingStep();
        ...
        equationText.append("];" + newLine);
        equationText.append("[t,y] = ode45(@eom, tspan, y0);" + newLine + newLine);
        ...
        if (SINGLE_PLOT) {
            ...
            equationText.append(plotMultipleFigures(substanceList, substanceMap));
        }
        ...
        for (int i=0; i<substanceList.length; i++){
            ...
            for (int j=0; j<substanceList.length; j++){
                ...
                exp = exp.replaceAll(concen,"y(" + substanceMap.get(temp.getName()) + ")");
                }
            }
            exp = checkExp(exp);
    ...
    private String plotMultipleFigures(Object[] substanceList, HashMap<String, Integer> subMap) {
        String newLine = System.getProperty("line.separator");
        ...
        for (int i=0; i<substanceList.length; i++) {
            ...
            text.append(newLine + "xlabel 'Time'; ylabel '" + s.getName() + "';" + newLine);
    ...
    private String plotSingleFigure(Object[] substanceList, HashMap<String, Integer> subMap) {
        String newLine = System.getProperty("line.separator");
        StringBuilder text = new StringBuilder();
    ...
    private String generateFunctionAppendix() {
        ...
        if (HAS_TRIGGERAT){
            ...
            appendix.append("    r = (trig).*t + (~trig).*d;" + newLine + newLine);
        }
        if (HAS_HILL) {
            appendix.append("function r = hill(c, nH, KH)" + newLine);
            appendix.append("    r = (c.^nH)./(c.^nH + KH.^nH);" + newLine + newLine);
        }
    ...
    protected void saveAsMfile(){
        String workingDir = dynetica.util.DyneticaProperties.getProperty("workingDirectory");
        JFileChooser chooser = new JFileChooser(workingDir);
        chooser.setAcceptAllFileFilterUsed(true);
        
        chooser.setFileFilter(new javax.swing.filechooser.FileFilter() {
            public boolean accept(File pathname) {
                return pathname.getName().endsWith(".m") || pathname.isDirectory();
            }
