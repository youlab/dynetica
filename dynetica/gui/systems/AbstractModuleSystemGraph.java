...
public class AbstractModuleSystemGraph extends javax.swing.JPanel 
   ...
   //
    ...
    protected void reset() {
       if (substanceNodes != null)
        ...
        expressionNodesForSuperSystem= expressionNodeMapForSuperSystem.values().toArray();
        ...
        reactionNodesForSuperSystem= reactionNodeMapForSuperSystem.values().toArray();
...
public void resetReaction(Reaction reaction){
    if(reaction instanceof ProgressiveReaction){
        ...
        String stoichiometry=((ProgressiveReaction)reaction).getStoichiometry();
        ...
        }catch(Exception exp){System.out.println("Kinetics is null");}
...
//no longer used    
    ...
    public void drawGraph(Graphics2D g2, int w, int h, int offsetX, int offsetY) {
        ...
        for (int i = 0; i < dottedsubstanceNodes.length; i++) {
            sn = (AbstractNode) dottedsubstanceNodes[i];
               if (sn.getX()<layout.margin) {
                 double theta = ((double) i / dottedsubstanceNodes.length) * 2 * pi ;
                 double y = radiusS * Math.sin(theta) * h / w + centerY;
                 double x = radiusS * Math.cos(theta) + centerX;
                 setLocation(sn, x, y);
            }
        ...
        for (int i = 0; i < dottedexpressionNodes.length; i++) {
            ...
            if (exp.getX()<layout.margin) {
                    double theta = ((double) i / dottedexpressionNodes.length) * 2.0 * pi + pi/2.718 ;
                    double y = radiusE * Math.sin(theta) * h / w + centerY;
                    double x = radiusE * Math.cos(theta)  + centerX;
                    setLocation(exp, x, y);
            }
        ...
        for (int i = 0; i < dottedreactionNodes.length; i++) {
          AbstractNode  rn = (AbstractNode) dottedreactionNodes[i];
            if (rn.getX()<layout.margin) {
                    double theta = ((double) i / dottedreactionNodes.length) * 2.0 * pi + pi/2.718 ;
                    double y = radiusR * Math.sin(theta) * h / w + centerY;
                    double x = radiusR * Math.cos(theta)  + centerX;
                    setLocation(rn, x, y);
            }
            ...
            for (int j = 0; j < reactants.size(); j++) {
                ...
                { 
                  if (sn != null ) {
                    double x2 = sn.getCenterX();
                    double y2 = sn.getCenterY();
                    new ArrowedLine(x2, y2, x1, y1, layout.arrowSize, 0.4, drawArrows, true).draw(g);
            ...
            for (int j = 0; j < products.size(); j++) {
                ...
                { 
                  if (sn != null ) {
                    double x2 = sn.getCenterX();
                    double y2 = sn.getCenterY();
                    new ArrowedLine(x1, y1, x2, y2, layout.arrowSize, 0.4, drawArrows, true).draw(g);
                     dottedsubstanceNodeMap.put(s, sn);
                     dottedsubstanceNodes=dottedsubstanceNodeMap.values().toArray();
                  }       
                }
 ...
 public void changeNodeSize(double ratio) {
    ...
    public class GraphMouseMotionListener implements MouseMotionListener {
        ...
        public void mouseDragged(MouseEvent e){
            if (currentNode != null) {
                ...
                if (y > getHeight()){ y = 0;
                //Kanishk Asthana 23 July 2013 : So that entity reorganises in a circle
                 x=-1.0*x;
    ...
    protected void removeGesture(){
                ...
                try{
                 if(system instanceof AbstractModule){
                  ((AbstractModule)system).removeEntityFromTreeModel(currentNode.getEntity());
                 }
                }
                ...
                try{
                 if(system.getSystem() instanceof ModularSystem ){
                  ((ModularSystem)system.getSystem()).insertEntityIntoTreeModel(currentNode.getEntity());
                 }
                }
	...
	drawGraph((Graphics2D) g, w, h,(int) pf.getImageableX(), (int) pf.getImageableY());
        restoreCoordinates();
