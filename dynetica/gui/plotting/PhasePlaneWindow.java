...
/**
 * August 2013. This is updated to use the more functional InteractiveFigure class (wrapping the XYplot from
 * the GRAL library.
 * @author lingchong
 */
...
//    final int nSubs;
    ...
    public PhasePlaneWindow(dynetica.system.ReactiveSystem system) {
        ...
        this.system = system;
        List subsAndExprs = system.getSubstances(); //this returns all the substances and expresion variables
        
        for (int i = 0; i < subsAndExprs.size(); i ++) {
            Substance s = (Substance) (subsAndExprs.get(i));
            if (!(s instanceof ExpressionVariable)) substances.add(s);    
        }
     ...
     public final void updateFigure() {
        ...
        if (xIndex > 0 && xIndex <= nSubs ) {
            ...
            ExpressionVariable expr = (ExpressionVariable) (expressions.get(xIndex - 2 * nSubs - 1));
        ...
        for (int i = 0; i < indices.length; i ++) {
            ...
            //otherwise, get rates instead of values
                ...
                ExpressionVariable expr = (ExpressionVariable) ( expressions.get(indices[i] - 2 * nSubs));
                for (int j = 0; j < dataPoints; j ++) {
                    double exprJ =  expr.getValues() [(int) Math.floor(j * samplingStep)];
                    yValues[i][j] = exprJ;                
                }
            }
