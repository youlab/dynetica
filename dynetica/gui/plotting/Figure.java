...
public class Figure extends JPanel {
    ...
    boolean logY = false;
    String[] yLabels;
    NumberFormat xFormat = NumberFormat.getNumberInstance();// new
                                                            // DecimalFormat("0.00");
    NumberFormat yFormat = NumberFormat.getNumberInstance(); // new
                                                             // DecimalFormat("0.00");
     ...
     * plotted against x[i].
     */
    public Figure(String xLabel, String[] labels, double[][] xv, double[][] yv) {
        super();
    ...
    public void setData(String xLabel, String[] labels, double[] xv,
            double[][] yv) {
        data = new DataModel(xv, yv);
        this.xLabel = xLabel;
        yLabels = labels;
    ...
    public void paint(Graphics g) {
        ...
        if (data != null) {
            ...
            for (int i = 0; i < xTicValues.length; i++) {
                g.drawLine(xTicPositions[i], yBottom, xTicPositions[i],
                        yBottom - 5);
                g.drawString(xFormat.format(xTicValues[i]),
                        xTicPositions[i] - 5, yBottom + 20);
            }
            ...
            for (int i = 0; i < yTicValues.length; i++) {
                ...
                g.drawString(yFormat.format(yTicValues[i]), xLeft - 40,
                        yTicPositions[i]);
            }
            ...
            for (int i = 0; i < xPoints.length; i++) {
                ...
                g.drawString(yLabels[i], xRight + 5,
                        yPoints[i][yPoints[0].length - 1]);
            }
            g.setColor(Color.black);
