...
/**
 * This  is a revised implementation of Figure.java. It provides a wrapper for the XYPlot and related classes (Legend, Axis,
 * LineRenderer, etc) from the free java graphing library GRAL. It is meant to provide basic functionality for plotting 
 * time courses or results from sensitivity analysis.
 * 
 * To use this class, create a new instance and set up the data by using the plotData method. The method expects a label 
 * for the X-axis, a vector of labels for y-values (these labels are used to create legends), a double vector of xValues, and 
 * a double matrix of yValues.
 * Then embed getPlotPanel() in the content pane of a java swing class to display.
 * 
 * 
 * @author lingchong
 */
...
public class InteractiveFigure  {
    ...
    //
    // 8/10/2013. These are defined to allow connection to Matlab to quickly plot simulation data.
    //
    ...
    public InteractivePanel getPlotPanel() {return plotPanel;}

    public InteractiveFigure () {
    }
    

    public InteractiveFigure ( String xLabel,
                    String [] labels,
		    double [] xv,
		    double [][] yv) {
   ...
   /**
    * Plotting multiple y traces against one x variable (e.g. multiple time courses)
    * 
    * @param xLabel: label for the x values
    * @param labels: labels for the y variables
    * @param xv [j]:  j: index of x values
    * @param yv [i][j]:  i: index of y variables; j: index of y values for each y variable.
    */
    
   public void plotData(String xLabel,
                    String [] labels,
		    double [] xv,
                    double [][] yv) {
        ...
        this.xLabel = xLabel;
        this.yLabels = labels; 
        this.yLegends = labels; // with a single trace of x values, keep the yLegends the same as yLabels.
        plot.clear();
        ...
        for (int j = 0; j < xLength; j++) {
          ...
          if (xMin < 0) setLogX(false);
          if (yMin < 0) setLogY(false);
        ...
        for (int i = 0; i < yLength; i++) {                                    
            ...
            Color currentColor;
            if (i < colors.length) currentColor = colors[i];
            else currentColor = new java.awt.Color((float) Math.random(),(float) Math.random(), (float) Math.random());
            plot.getLineRenderer(dataSeries).setSetting(LineRenderer.COLOR, currentColor);
           
            PointRenderer pointRenderer = plot.getPointRenderer(dataSeries);
            pointRenderer.setSetting(PointRenderer.COLOR, GraphicsUtils.deriveDarker(currentColor));
               
            java.awt.Shape circle = new java.awt.geom.Ellipse2D.Double(-2.0, -2.0, 4.0, 4.0);
    ...
    * 
    * @param xLabels : labels of x variables
    * @param yLabels : labels of y variables
    * @param xv [i][j]   i: index of x variables; j: index of data points for each x variable
    * @param yv [i][k][j]  i: index of x variables; k: index of y variables; j: index of y data points for each y variable
    */
   
   public void plotData (String [] xLabels,
                    String [] yLabels,
		    double [][] xv,
                    double [][][] yv) {
        //reset minimal and maximum values
        ...
        if (yLabels != null) this.yLegends = new String[yLabels.length * numXVar];
        ...
        for (int i = 0; i < numXVar; i ++  ) {          
            ...
            for (int k = 0; k < numPoints; k ++) {
                ...
                //
                // also store the data to the overall dataTable for later retrival
                dataTable.add(dataRow);
            }
            ...
            for (int j = 0; j < numYVar; j++) {
                ...
                int colorIndex  = i;
                if (colorIndex < colors.length) currentColor = colors[colorIndex];
                else currentColor = new java.awt.Color((float) Math.random(),(float) Math.random(), (float) Math.random());
                plot.getLineRenderer(dataSeries).setSetting(LineRenderer.COLOR, currentColor);

                PointRenderer pointRenderer = plot.getPointRenderer(dataSeries);
                Color pointColor;
                if (j % 2 == 0) pointColor = GraphicsUtils.deriveDarker(currentColor);
                else pointColor = GraphicsUtils.deriveBrighter(currentColor);
                pointRenderer.setSetting(PointRenderer.COLOR, pointColor);
                int ShapeIndex  = j % shapes.length;
              //  java.awt.Shape circle = new java.awt.geom.Ellipse2D.Double(-2.0, -2.0, 4.0, 4.0);
                pointRenderer.setSetting(PointRenderer.SHAPE, shapes[ShapeIndex]);
   ...
   private void formatPlot() {
    ...
    private void formatAxis(){
        // Format axes
           ...
           // The following code is to avoid trying to plot data that don't have a significant range (Plotting 
           // can freeze the program)
           //
           if ((Math.abs(xMax - xMin) <= 10.0 * Double.MIN_VALUE) && (xMin <= 10.0 * Double.MIN_VALUE)) {
               xMin = 0.0;
               xMax = 1.0;
           }
           
           if ((Math.abs(yMax - yMin) <= 10.0 * Double.MIN_VALUE) && (yMin <= 10.0 * Double.MIN_VALUE)) {
               yMin = -1.0;
               yMax = 1.0;
           }
           ...
           if ((logX) && xMin >= 0.0) {
               ...
               double tickSpacing = Math.min(4.0, Math.log10(xMax/xLogMin) + 1.0);
          ...
          if ((logY) && yMin >=0.0) {
               ...
               double tickSpacing = Math.min(4.0, Math.log10(yMax/yLogMin) + 1.0);
    ...
    public void addLegends(String location){
        ...
        if (location.equals("EAST") || location.equals("WEST") ) {
            ...
            double legendSpace = 100.0 + 6.5 * maxYLegendsLength(); //needs to be fine tuned.
            if (location.equals("EAST")) {
                plot.setSetting(Plot.LEGEND_LOCATION, Location.EAST);
                plot.setInsets(new Insets2D.Double(20.0, 50.0, 60.0, legendSpace));
            }
            else {
                plot.setSetting(Plot.LEGEND_LOCATION, Location.WEST);
                plot.setInsets(new Insets2D.Double(20.0, legendSpace, 60.0, 15.0));
...
public void setXLabel(String xLabel) {
    this.xLabel = xLabel;
    plot.getAxisRenderer(XYPlot.AXIS_X).setSetting(AxisRenderer.LABEL, xLabel);
}
...
private int maxYLegendsLength () {
    int maxLength = 0;
    if (yLegends == null) return maxLength;
...
private int totalYLegendsLength () {
    int totalLength = 0;
    if (yLegends == null) return totalLength;
    for (int i = 0; i < yLegends.length; i++) {
//        System.out.println(yLabels[i]);
 ...
 */
public static void matlabPlot(String xLabel, 
        String yLabel, 
        double [] xValues,
        double [] yValues,
        boolean logX,
    ...
    if (matlabfactory == null)   matlabfactory = new MatlabProxyFactory();
    if (proxy == null) proxy = matlabfactory.getProxy();

    String xName = "X";
    String yName = "Y";

    if (xLabel != null) xName = xLabel;
    if (yLabel != null) yName = yLabel;
    ...
    if (xLabel != null) proxy.feval("xlabel", xLabel);
    if (yLabel != null) {
 ...
 * @param xLabel
 * @param yLabels
 * @param xValues [j]  j: index of x values
 * @param yValues [i][j]  i: index of y variables; j: index of data points for each y variable
 * @param logX
 * @param logY
 * @throws MatlabConnectionException
 * @throws MatlabInvocationException 
...
//    if (xLabel != null) xName = xLabel;
//    if (yLabels != null && yLabels.length == 1 ) yNames [0] = yLabels [0];
    ...
    for (int i = 0; i < yValues.length; i ++) {
       ...
       matlabPlot(xName, yNames[i], xValues, yValues[i], logX, logY, lineStyles[lineStyleIndex]);
    }
 ...
 * Plotting multiple y traces against multiple x traces.
 * 
 * @param xLabels -- its length corresponds to number of x variables 
 * @param yLabels -- its length corresponds to number of y variables for each x variable
 * @param xValues [i][j]  i: index of x variables; j index of data points for each x variables
 * @param yValues [i][k][j]  i: index of x variables; k index of y variables for each x variable; j: index of data points for each y variable
 * @param logX
 * @param logY
 * @throws MatlabConnectionException
 * @throws MatlabInvocationException 
 */
public static void matlabPlot(String[] xLabels, 
        String [] yLabels, 
        double [][] xValues,
        double [][][] yValues,
        boolean logX,
    ...
    if (xLabels!= null && xLabels.length != xValues.length) return;
    if (yLabels!= null && yLabels.length != yValues[0].length) return;
    ...
    for (int i = 0; i < xValues.length; i ++) {
       if (xLabels != null && yLabels != null) {
         for (int j = 0; j < yLabels.length; j++) yNames[j] = yLabels[j] + " vs " + xLabels[i];
         
         //
         // maybe better to call the single-trace plot for easier tracking of data and labels
         //
         matlabPlot(xLabels[i], yNames, xValues[i], yValues[i], logX, logY);           
       }
       else
