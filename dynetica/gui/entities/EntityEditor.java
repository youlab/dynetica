...
public class EntityEditor extends JPanel{
    ...
    try {
      entityInfo = Introspector.getBeanInfo(
        entity.getClass(), java.lang.Object.class);
      properties = 
      entityInfo.getPropertyDescriptors();
      ...
      try {
        ...
        if (properties[i].getName().compareTo("system") == 0) {
            if (entity.getSystem() != null)
                values[i] = new JTextField(entity.getSystem().getName());
            else
            ...
            Object currentValue = properties[i].getReadMethod().invoke(entity, null);
            if (currentValue != null) 
               values[i] = new JTextField(currentValue.toString());
            else
               values[i] = new JTextField();
 ...
 */
  ...
  public static class Tester {
     public static void main(String[] args) {
        dynetica.system.ReactiveSystem rs = new dynetica.system.ReactiveSystem("RS");
        dynetica.entity.Substance en = new dynetica.entity.Substance();
        ...
        dynetica.reaction.ProgressiveReaction pr = new 
            dynetica.reaction.ProgressiveReaction("TestReaction", rs);
        try {
        ...
        EquilibratedMassAction ema = new EquilibratedMassAction ("TestReaction", rs);
        try {
            ema.setProperty("stoichiometry", "A + 3 B -> C + 2 D");
            ema.setProperty("K", "10000");
     ...
     //       ema.printStatus();
         ...
         Parameter vmax = new Parameter("vmax",rs, 10, 10, 10);
	 Parameter km = new Parameter("Km",rs, 20, 20, 20);
         MichaelisMentenReaction r = new 
	     MichaelisMentenReaction("MichaelisMentenReaction", rs, vmax, km);
         Substance e = new Substance("E", rs);
         Substance s = new Substance("S", rs);
         Substance p = new Substance("P", rs);
         try {
         r.setProperty("substrate", "S");
         r.setProperty("enzyme", "E");
         r.setProperty("product", "P");
         
         e.setValue(0.01);
         //h2.setConcentration(1.5);
         s.setValue(0.01);
