...
public class ArrowedLine extends Line2D.Double {
    ...
    public ArrowedLine(double x1, double y1, double x2, double y2, double size,
            double angle, boolean arrowVisible, boolean dashed) {
        super(x1, y1, x2, y2);
        this.arrowSize = size;
    ...
    public ArrowedLine(double x1, double y1, double x2, double y2, double size,
            double angle, double arrowPosition, boolean arrowVisible) {
        super(x1, y1, x2, y2);
        this.arrowSize = size;
        this.arrowAngle = angle;
    ...
    private void drawArrow(Graphics2D g) {
        if (arrowVisible) {
            ...
            double alpha = Math.asin((y2 - y1)
                    / Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1)));
            double absoluteAngle1;

            if (x2 > x1)
                absoluteAngle1 = pi + (alpha - arrowAngle);
            else
                absoluteAngle1 = arrowAngle - alpha;

            double firstArrowTailX = arrowHeadX + arrowSize
                    * Math.cos(absoluteAngle1);
            double firstArrowTailY = arrowHeadY + arrowSize
                    * Math.sin(absoluteAngle1);
            double absoluteAngle2;
            if (x2 > x1)
                absoluteAngle2 = pi + (alpha + arrowAngle);
            else
                absoluteAngle2 = -arrowAngle - alpha;

            double secondArrowTailX = arrowHeadX + arrowSize
                    * Math.cos(absoluteAngle2);
            double secondArrowTailY = arrowHeadY + arrowSize
                    * Math.sin(absoluteAngle2);

            g.draw(new Line2D.Double(firstArrowTailX, firstArrowTailY,
                    arrowHeadX, arrowHeadY));
            g.draw(new Line2D.Double(secondArrowTailX, secondArrowTailY,
                    arrowHeadX, arrowHeadY));
        }
    }
    ...
    public void draw(Graphics2D g) {
        if (dashed) {
            ...
            BasicStroke stroke = new BasicStroke(oldStroke.getLineWidth(),
                    dashedStroke.getEndCap(), dashedStroke.getLineJoin(),
                    dashedStroke.getMiterLimit(), dashedStroke.getDashArray(),
