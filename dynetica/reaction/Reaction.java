...
abstract public class Reaction extends Entity{
    ...
    public List getReactants() { return reactants;}
    public List getProducts() { return products;}
    public List getCatalysts() { return catalysts;}
    public Set getSubstances() { return substances.keySet();}
	...
	if (coefficient.doubleValue() < 0.0 ) reactants.add(s);
	else if (coefficient.doubleValue() > 0.0) products.add(s);
	else catalysts.add(s);
    ...
    /** Update the concentration of all the substances of a reaction
     *  based on the time step and the rate of the reaction
     */
  //  abstract public void update(double dt) throws RateExpressionNotSetupException;

    /** 
     * Update the reaction by one molecular event. Consider the following
     * reaction: 
     *  A (10)  + B (10)  -> C (20) with the numbers of molecules indicated
     *  in the parentheses. After one firing of the reaction, the system
     * becomes:
     *  A (9) + B (9) -> C (21).
     *
     * This function is probably only useful when we are using stochastic algorithms,
     * such as the one proposed by Gillespie, to simulate the coupled chemical reactions.
     */
    public void fire() {
     ...
     * @param stoichiometry New value of property stoichiometry.
     */
...
//        System.out.println(stoichiometry);
       ...
       while(itr.hasNext()) {
	    ...
	    Double coefficient = (Double) reactionParser.getStoichiometry().get(substanceName);
	    //
		...
		addSubstance(getSystem().getSubstance(substanceName), coefficient);
	    //
	    // if not, creat a substance with that name and insert into the reaction
	    // as well as into the system.
	    //
	    else {
                System.out.println("Warning:" + substanceName  + " not found in system " + 
                                    getSystem().getName() + NEWLINE +
                                    "New substance with this name is created." );
