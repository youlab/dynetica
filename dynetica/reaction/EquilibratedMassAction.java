...
public class EquilibratedMassAction extends EquilibratedReaction {
    ...
    /** Holds value for the equilibrium constant kExpression. This
     constant could actually be a complexity algebraic expression*/
    ...
    public void reset() {
             ...
             //
             // note only the coefficients of reactants and products are initialized here.
             //
             for (int i = 0; i < reactantValues.length; i++) 
                 // converted to positive values
                 reactantCoefficients[i] = - getCoefficient((Substance) reactants.get(i));
     ...
     throws UnknownPropertyException, InvalidPropertyValueException {
         ...
         //the following two IF statements were added 4/19/05 by LY 
           ...
           setY(Double.parseDouble(propertyValue));
            ...
            throw 
            new UnknownPropertyException("Unknown property for EquilibratedMassAction:" + propertyName);
         }
     }
    ...
    // based on the current concentrations of the reacting species, using a trial
    // and error method.
    // 
    public void update() {
 ...
 //       System.out.println(kvalue);
        ...
        double minReactantEps = Double.MAX_VALUE; // = min([Product_i]/[Product_i_coeff])
        double minProductEps = Double.MAX_VALUE;  // = min([Reactant_i]/[Reactant_i_coeff]);
        
        for (int i = 0; i < reactantValues.length; i++) {
            reactantValues[i] = ((Substance) reactants.get(i)).getValue();
            minReactantEps = Math.min(minReactantEps, reactantValues[i]/reactantCoefficients[i]);
        }
                
        for (int j = 0; j < productValues.length; j++) {
            productValues[j] = ((Substance) products.get(j)).getValue();
            minProductEps = Math.min(minProductEps, productValues[j]/productCoefficients[j]); 
        }
 ...
 //       System.out.println(minEps);
     ...
     //       System.out.println(eps);
            double kprime = 1.0;
            for (int i = 0; i < productValues.length; i ++) 
                kprime *= Math.pow((productValues[i] + eps * productCoefficients[i]), productCoefficients[i]);
            for (int i = 0; i < reactantValues.length; i ++)
                kprime *= Math.pow((reactantValues[i] - eps * reactantCoefficients[i]), -reactantCoefficients[i]);
        ...
        for (int i = 0; i < reactantValues.length; i ++) 
           ((Substance) reactants.get(i)).setValue(reactantValues[i] - reactantCoefficients[i] * eps);
        
        for (int i = 0; i < productValues.length; i ++) 
           ((Substance) products.get(i)).setValue(productValues[i] + productCoefficients[i] * eps);
    ...
    public void setK(String kString) throws IllegalExpressionException {
        this.k = kString;
        try {
            this.kExpression = ExpressionParser.parse(this.getSystem(), kString);
        }
        catch (IllegalExpressionException iee) {
            throw iee;
        }
    ...
    public Object clone(){
        EquilibratedMassAction ema = new EquilibratedMassAction (
                this.name + "_clone",
