...
public class ReactionParser  {
    ...
    /**
     * represents the string or character that separates the reactants and products.
     */
    String separator; 
    ...
    public Map getStoichiometry() { return stoichiometry;}

    public void parseReaction() {
        // clear the map before doing anything.
        if (!stoichiometry.isEmpty()) stoichiometry.clear();
	int indSeparator = reactionEquation.indexOf(separator);
	String rString = (reactionEquation.substring(0, indSeparator)).trim();
	String pString = (reactionEquation.substring(indSeparator + separator.length())).trim();
	if (rString.length() > 0) process(rString, true);
	if (pString.length() > 0) process(pString, false);
    }
	...
	while (strTokens.hasMoreTokens()) {
		...
		//
		index ++;
		while( Character.isDigit(element.charAt(index)) || 
		       element.charAt(index) == '.') index++;
		coefficient = Double.parseDouble(element.substring(0, index));
		...
		if(tempName.length() > 1 && (tempName.charAt(0) == 'e' ||
			tempName.charAt(0) == 'E' )&& tempName.charAt(1) == '-'){
			tempIndex = 2;
			while( Character.isDigit(tempName.charAt(tempIndex)) || 
		       element.charAt(tempIndex) == '.') tempIndex++;
		...
		} else if(tempName.length() > 1 && (tempName.charAt(0) == 'e' ||
		tempName.charAt(0) == 'E' )&& Character.isDigit(tempName.charAt(1))){
			tempIndex = 1;
			while( Character.isDigit(tempName.charAt(tempIndex)) || 
		       tempName.charAt(tempIndex) == '.') tempIndex++;
		...
		tempName.charAt(0) == 'E' )){
			if(!strTokens.hasMoreTokens())
				...
				if(Character.isDigit(subSt_next.charAt(0))){
					tempIndex = 0;
					while(Character.isDigit(subSt_next.charAt(tempIndex)) || 
		       			subSt_next.charAt(tempIndex) == '.') tempIndex++;
	    ...
	    if (isReactants) coefficient = - coefficient;
	    //
     ...
     * @param reactionEquation New value of property reactionEquation.
     */
