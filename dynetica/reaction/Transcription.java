...
public class Transcription extends ProgressiveReaction {
    ...
    Gene gene = null; // gene will hold the information about the rate
                      // expression and its mRNA
    Substance NTP = null;
    ...
    /**
     * set up the transcription reaction: its reaction formula, and its rate
     * expression the rate is calculated based on the number of NTPs.
     */
    public void setup() {
        clear();
        RNA rna = gene.getRna();
        if (NTP != null)
            addSubstance(NTP, -1.0);
        addSubstance(gene.getRNAP(), 0.0);
        addSubstance(rna, 1.0 / (gene.getLength()));
        rateExpression = DMath.multiply(gene.getRNAP().getElongationRate(),
                DMath.min(DMath.multiply(gene.getRNAP(), DMath.divide(DMath
                        .multiply(gene.getKTranscription(), gene), gene
                        .getRNAP().getTotalTranscriptionActivity())),
                //
                // the maximum # of RNAPs that can be allocated to a gene
                // is the gene length divided by the RNAP spacing requirement.
                //
                        DMath.divide(new Constant(gene.getLength()), gene
                                .getRNAP().getSpacing())));

        // LY:
        // The following code sets the rate expression to be somewhat like the
        // Michaelis Menten kinetics. The constant 1e3 is a value arbitrarily
        // chosen
    ...
    public String toString() {
        return (getFullName() + "{" + NEWLINE + "\t Gene { "
                + gene.getLongName() + " } " + NEWLINE + "\t stoichiometry { "
