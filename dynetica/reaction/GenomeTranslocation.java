...
//             System.out.println(" "+ len + "  " + lin);
             ...
             //
             // search for the first element whose "tail" is still outside of the cell.
             //
             for (int i = 0; i < elements.size() ; i++) {
                 ge = (GeneticElement) elements.get(i);
                 if (ge.getEnd() > lin) break;
                 if (! ge.isActive()) ge.setActive(true);
             }
        }       
    }
     ...
     throws UnknownPropertyException, InvalidPropertyValueException {
         if (propertyName.compareToIgnoreCase("kinetics") == 0) {
             ...
             try {
                 this.rateExpression = ExpressionParser.parse(this.getSystem(), propertyValue);
             }
            ...
            throw 
            new UnknownPropertyException("Unknown property for GenomeTranslocation:" + propertyName);    
     }
