...
public class NoiseOptimizationCrossTrial implements Runnable {
    ...
    public NoiseOptimizationCrossTrial(ReactiveSystem sys, Parameter[] p,
            Substance s, double tol, double time, int reps, int iters) {
        system = sys;
        searchParameters = p;
        targetSubstance = s;
        varTolerance = tol;
    ...
    public void run() {
        ...
        if ((algorithm instanceof RungeKutta4)
                || (algorithm instanceof RungeKuttaFehlberg)) {
            JOptionPane
                    .showMessageDialog(system.editor(),
                            "You must use a stochastic algorithm for noise optimization.");
            return;
        }

        // Initialization

        double[][] substanceValues = new double[numRepetitions][algorithm
                .getIterations()];
        double[][] timeValues = new double[numRepetitions][algorithm
                .getIterations()];
        ...
        double ssMean = Statistics
                .mean(average, startIndex, average.length - 1);
        originalMean = ssMean;
        ...
        double avgVariance = Statistics.mean(variances, startIndex,
                variances.length - 1);
        ...
        while (true) {
            ...
            substanceValues = new double[numRepetitions][algorithm
                    .getIterations()];
             ...
             * double[numRepetitions][algorithm.getIterations()]; for (int i =
             * 0; i < average.length; i++){ double avg = average[i]; for (int j
             * = 0; j < numRepetitions; j++){ difference[j][i] =
             * substanceValues[j][i] - avg; } }
             */
            variances = new double[average.length];
            for (int c = 0; c < variances.length; c++) {
                variances[c] = Statistics.var(getColumn(substanceValues, c));
            }
            avgVariance = Statistics.mean(variances, startIndex,
                    variances.length - 1);
            ...
            System.out.println("Mean = " + ssMean + ", Var = " + avgVariance
                    + ", Score = " + score);

            boolean acceptableMeanChange = true;
            if (limitMean) {
                if (ssMean > originalMean * (1 + meanTolerance / 100)) {
                    acceptableMeanChange = false;
                }
                if (ssMean < originalMean * (1 - meanTolerance / 100)) {
                    acceptableMeanChange = false;
                }
            ...
            double improvementThreshold = ExpressionConstants.doubleNumber
                    .nextGaussian() / STRICT_FACTOR;
            if (acceptableMeanChange && (improvement > improvementThreshold)) {
                prevScore = score;
                continue;
            }
            if (!acceptableMeanChange) {
                System.out.print("Too much change in mean.  ");
            }
                ...
                if (ExpressionConstants.doubleNumber.nextDouble() < MUTATION_PROB) {
                    ...
                    if (Math.abs(value) < .001) {
                        ...
                        changes[i] = value
                                * Math.pow(
                                        10,
                                        ExpressionConstants.doubleNumber
                                                .nextGaussian() / SCALE_FACTOR);
                    }
                    c++;
                } else {
                    changes[i] = 0;
    ...
    public void setMinimizing(boolean m) {
        minimize = m;
    }
    ...
    public void setIsSNR(boolean s) {
        SNR = s;
    }

    public boolean getLimitingMean() {
        return limitMean;
    }

    public void setLimitingMean(boolean l) {
        limitMean = l;
    }
    ...
    public void setMeanTolerance(double t) {
        meanTolerance = t;
    }
