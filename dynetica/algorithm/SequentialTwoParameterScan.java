...
public class SequentialTwoParameterScan implements Runnable {
    ...
    java.io.File output = null; // file for storing time course data if saveData
                                // is true.
    ...
    double[][][] yValues; // stores the final values of different objective
                          // functions
    // double[][][][] timeCourses; //stores the time courses of objective
    // functions resulting from the different variable values
    ...
    public SequentialTwoParameterScan(ReactiveSystem sys, java.io.File out) {
        system = sys;
        output = out;
    ...
    public void run() {
        if (numPoints1 <= 0 || numPoints2 <= 0 || variable1 == null
                || variable2 == null || metricList == null) {
            System.out
                    .println("Error in algorithm setup (sequential parameter scan). Aborting. ");
            return;
        }
        ...
        x1Values = null;
        x2Values = null;
        EntityVariable sysVariable1 = (EntityVariable) (system
                .getEntity(variable1.getName()));
        EntityVariable sysVariable2 = (EntityVariable) (system
                .getEntity(variable2.getName()));
        yValues = new double[numMetrics][numPoints1][numPoints2];

        if (variable1.isLogScale())
            x1Values = dynetica.util.Numerics.logspace(variable1.getMin(),
                    variable1.getMax(), numPoints1);
        else
            x1Values = dynetica.util.Numerics.linspace(variable1.getMin(),
                    variable1.getMax(), numPoints1);

        if (variable2.isLogScale())
            x2Values = dynetica.util.Numerics.logspace(variable2.getMin(),
                    variable2.getMax(), numPoints2);
        else
            x2Values = dynetica.util.Numerics.linspace(variable2.getMin(),
                    variable2.getMax(), numPoints2);

        // timeCourses = new double[numVariables][numMetrics][numPoints +
        // 1][numTimePoints];

        double oldValue1 = sysVariable1.getValue();
        double oldValue2 = sysVariable2.getValue();

        for (int j = 0; j < x1Values.length; j++) { // scanning through 1st
                                                    // variable
            ...
            for (int i = 0; i < x2Values.length; i++) { // 2nd layer scanning
                                                        // through individual
                                                        // parameter values

                double currentValue2 = x2Values[i];
                //
                // Change the value or initial value of the correponding system
                // variable (parameter or substance)
                //
            ...
            //
            // Reset the value or initial values of the correponding system
            // variables (parameter or substance)
            //
            if (sysVariable1 instanceof Parameter)
        ...
        if (plotOnComplete) {
            ...
            // new
            // dynetica.gui.plotting.MultiParameterSensitivityPlaneWindow(this);
            // s.show();
        }
    ...
    public ArrayList<AbstractMetric> getMetrics() {
        return metricList;
    }

    public double[] getX1Values() {
        return x1Values;
    }

    public double[] getX2Values() {
        return x2Values;
    }
    ...
    public int getNumPoints1() {
        return numPoints1;
    }

    public int getNumPoints2() {
        return numPoints2;
    }
    ...
    public void setNumPoints1(int i) {
        numPoints1 = i;
    }

    public void setNumPoints2(int i) {
        numPoints2 = i;
    }

    public void setMetrics(ArrayList<AbstractMetric> metrics) {
        metricList = metrics;
    }

    public void setPlotOnComplete(boolean b) {
        plotOnComplete = b;
    }
