...
public class SensitivityAnalysis implements Runnable {
    ...
    public SensitivityAnalysis(Variable v, ArrayList<AbstractMetric> list,
            ReactiveSystem sys, double minimum, double maximum, int num,
            double simTime, boolean log) {
        variable = v;
        metricList = list;
        system = sys;
        min = minimum;
    ...
    public void run() {
        if (numSimulations <= 0 || variable == null || metricList == null) {
            System.out.println("Error in parameters. NumSim = "
                    + numSimulations + " Variable = " + variable
    ...
    public void setNumSimulations(int i) {
        numSimulations = i;
    }
