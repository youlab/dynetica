...
public class BasicSensitivityAnalysis implements Runnable{
    ...
    private Substance [] substances = null; // selected substances to be evaluated.
    ...
    private double[][][] timeCourses; // timeCourse[i][j][k].  i: index of y variables, j: index of parameter value, k: index of timepoints 
     ...
     * @param cycles New value of property cycles.
     */
    ...
    public void run() {
        ...
        if (cycles > 0 && variable != null && substances != null) {
            ...
            timeCourses = new double[substances.length][cycles + 1][algorithm.getIterations()];
            ...
            for (int i = 0; i <= cycles; i++) {
                ...
                if (!logScale) {
                    ...
                    currentValue = Math.exp(i * (Math.log(max) - Math.log(min))/cycles + Math.log(min));
                }
