...
public class OptimizedDirectMethod extends DirectMethod {
    ...
    public OptimizedDirectMethod(ReactiveSystem system,
		      double samplingStep,
		      int iterations,
                      int numberOfRounds,
                      java.io.File output) {
 ...
 //               totalActivity += a[i];
        ...
        for (int i = 0; i < exprs.size(); i ++ ){
            ...
            for (int j = 0; j< temp.size(); j++) {
                ...
                indices[numberOfUpdates++] = ((Integer) (reactionMap.get(pr))).intValue();
            }
        }
    ...
    private void setUpdateList(Reaction r){
       ...
       // System.out.println(numberOfUpdates);
        for (int i = 0; i < reactionsToUpdate.size(); i++) {
            ProgressiveReaction pr = (ProgressiveReaction) (reactionsToUpdate.get(i));
                indices[numberOfUpdates++] = ((Integer)(reactionMap.get(pr))).intValue();
            }
