 ...
 * @author Derek Eidum
 */
public class ParameterSearchDoseResponse implements Runnable {
    ...
    ArrayList<TargetDoseObjective> objectives;
    ArrayList<ParameterVector> population;
    
    double [] originalParameterValues;
    
    Variable variable;
    double[] xValues; // The dose (if dose response) or parameter (if sensitivity analysis)
    ...
    public ParameterSearchDoseResponse(ReactiveSystem s, Parameter[] p, ArrayList<TargetDoseObjective> tdo,
                Variable v, double[] xvals, double time, double th, int g, int pop, boolean r) {
        system = s;
        ...
        // set the dose variable for all objective functions
        for (TargetDoseObjective o : objectives) {
            o.getObjective().setDose(xvals);
        }
    }
        ...
        for (int generation = 0; generation <= numGenerations; generation++){
            ...
            if (generation==numGenerations){
                String message = "Maximum generations reached.\nBest results shown below.\n" + 
                        stringResult(alphaDog) + "Click Run again to continue this search.";
    ...
    public void scorePopulation() {
        for (ParameterVector pv : population) {
            ...
            // Calculate each metric for every dose
            double[][] metricValues = new double[objectives.size()][xValues.length];
            for (int i = 0; i < xValues.length; i++) {
                setVariable(xValues[i]);
                compute();
                for (int j = 0; j < objectives.size(); j++) {
                    metricValues[j][i] = objectives.get(j).getMetric().getValue();
                }
            }
            
            // Set the objective data for scoring
            for (int j = 0; j < objectives.size(); j++) {
                objectives.get(j).getObjective().setResponse(metricValues[j]);
            }
            
            // Take a weighted average of all the objective scores
            double score = 0;
            for (TargetDoseObjective obj : objectives){
    ...
    public void setVariable(double d) {
        if (variable instanceof Substance) {
            ((Substance) variable).setInitialValue(d);
        } else if (variable instanceof Parameter) {
            ((Parameter) variable).setValue(d);
        }
    }
    ...
    public Thread getSimulationThread() {return simulationThread;}
    public ReactiveSystem getSystem() {return system;}
    public ArrayList<TargetDoseObjective> getObjectives() {return objectives;}
