...
public class ParameterSearchMonteCarlo implements Runnable {
    ...
    public ParameterSearchMonteCarlo (ReactiveSystem sys, Parameter[] p, Substance s, double t, double time) {
        system = sys;
        searchParameters = p;
        targetSubstance = s;
        targetValue = t;
    ...
    public void run() {
        ...
        while (true) {
            ...
            while (c <= 0){
                for (int i = 0; i < searchParameters.length; i++){
                    if (ExpressionConstants.doubleNumber.nextDouble() < ((float) numAlterations)/changes.length){
                        changes[i] = calculateChange(searchParameters[i], Math.abs(prevError)/targetValue);
                        c++ ;
                    } else {
                        changes[i] = 0;
                    }
            ...
            double relativeImprovement = (Math.abs(prevError) - Math.abs(error))/targetValue;
    ...
    public double calculateChange(Parameter p, double relError) {
        ...
        return Math.pow(10,ExpressionConstants.doubleNumber.nextGaussian()/(25) * relError) * value;
    }
    ...
    public double getThreshold(double d) { return threshold; }
