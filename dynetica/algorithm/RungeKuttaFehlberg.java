...
/**
 * RungeKuttaFehlberg implements an adaptive step 4th order
 * Runge-Kutta algorithm invented by Fehlberg. The following implementation
 * is from Numeric Receipes (c) 1988-1992 by Cambridge University Press.
 * (p714 - p722).
 * 
...
public class RungeKuttaFehlberg extends Algorithm {
     ...
     *
     *  h1 = SAFETY * h0 * (error / tolerance) ^ PGROW to grow the time step 
     *  h1 = SAFETY * h0 * (error / tolerance) ^ PSHRINK to shrink the time step
     */
    ...
    int maxIterations;  // the maximum number of iterations per samplingStep allowable.
    ...
    double []  yerr; // Error estimate as the difference between 4th and 5th order methods.
    ...
    public RungeKuttaFehlberg(ReactiveSystem system,
                              int iterations,
                              double samplingStep,
                              double initialStep,
                              double tolerance, 
	...
	timer = system.getTimer();
      ...
      private void rkckStep(double h) {
		...
		k5[i] = h * s[i].getRate();
            ...
            // step 6
            // k6 = h * f (t + a6 * h, y + b61 * k1 + b62 * k2 + b63 * k3 + b64 * k4 + k65 * k5)
            //
            timer.setTime(t + a5 * h);
 	    for (int i = 0; i < numberOfSubstances; i++) {
		...
		k6[i] = h * s[i].getRate();
            ...
            for (int i = 0; i < numberOfSubstances; i++) {
                y2[i] = y1[i] + c1 * k1[i] + c3 * k3[i] + c4 * k4[i]  + c6 * k6[i]; //fifth order estimate
    
                //
                // error between 4th and 5th 
                //
                // note that the errors are scaled with respect to estimated new values
                // if the new values are smaller than 1e-8, scale to 1e-8. This number is
                // used to avoid scaling with respect to a number that is too small.
                //
                yerr[i] = Math.abs((dc1 * k1[i] + dc3 * k3[i] + dc4 * k4[i] + dc5 * k5[i] + dc6 * k6[i]) / 
        ...
        public void update() {
            ...
            for (;;) {
	         ...
	         t += step;
                 updateSystem(step);
                //
                // grow the step a little bit. but not more than a factor of five
                //
...
//              timer.step(step);
     ...
     * @param v  Value to assign to tolerance.
     */
 ...
 */
     ...
     * @param initialStep New value of property initialStep.
 */
 ...
 */
     ...
     * @param minStep New value of property minStep.
 */
