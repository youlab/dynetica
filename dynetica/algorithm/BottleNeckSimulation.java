...
public class BottleNeckSimulation implements Runnable {
    ...
    public void run() {
        if (saveData){
            ...
            if (i > 0) {folderName = folderName + i;}
            boolean success = folder.mkdirs();
            if (!success) {
                System.out.println("Failed to create necessary folders. Saving disabled.");
                saveData = false;
            }
        }
        ...
        double[][] cooperatorValues = new double[subpopulationSize+1][algorithm.getIterations()];
        double[][] cheaterValues = new double[subpopulationSize+1][algorithm.getIterations()];
        ...
        for (int run = 0; run < numRounds; run++) {
            ...
            BinomialDistribution bd = new BinomialDistribution(subpopulationSize, initialCheaterFraction);
            for (int cheatCount = 0; cheatCount <= subpopulationSize; cheatCount++) {
                simulationWeight[cheatCount] = bd.probability(cheatCount);
                if (normalizeIV) {
                    cheater.setInitialValue(normalizedIV * cheatCount / subpopulationSize);
                    cooperator.setInitialValue(normalizedIV * (1 - cheatCount/subpopulationSize));
                } else {
                ...
                for (int i = 0; i < algorithm.getIterations(); i++) {
                    globalCheaterValues[i] += cheaterValues[cheatCount][i]*simulationWeight[cheatCount];
                    globalCooperatorValues[i] += cooperatorValues[cheatCount][i]*simulationWeight[cheatCount];
                }
            }
            ...
            initialCheaterFraction = (finalCheater)/(finalCheater + finalCooperator); 
        }
    ...
    public void runPoisson() {
        ...
        for (int run = 0; run < numRounds; run++) {
            ...
            for (int pop = 1; pop <= maxSize; pop++) {
                ...
                double[][] cooperatorValues = new double[pop+1][algorithm.getIterations()];
                double[][] cheaterValues = new double[pop+1][algorithm.getIterations()];
                double poissonWeight = pd.probability(pop);
                BinomialDistribution bd = new BinomialDistribution(pop, initialCheaterFraction);
                for (int cheatCount = 0; cheatCount <= pop; cheatCount++) {
                    simulationWeight[cheatCount] = bd.probability(cheatCount);
                    if (normalizeIV) {
                        cheater.setInitialValue(normalizedIV * cheatCount / pop);
                        cooperator.setInitialValue(normalizedIV * (1 - cheatCount/pop));
                    } else {
                        cheater.setInitialValue(cheatCount);
                    ...
                    cheaterValues[cheatCount] = cheater.getValues();
                    for (int i = 0; i < algorithm.getIterations(); i++) {
                        globalCheaterValues[i] += cheaterValues[cheatCount][i]*simulationWeight[cheatCount]*poissonWeight;
                        globalCooperatorValues[i] += cooperatorValues[cheatCount][i]*simulationWeight[cheatCount]*poissonWeight;
                    }
                }
            }
            double[] timeVals = system.getTimer().getTimePoints();
    ...
    private void compute() {
        algorithm = system.getAlgorithm();
        algorithm.reset();
        algorithm.setSamplingStep(subsimulationTime / algorithm.getIterations());
    ...
    private void saveData(int run, int cheatCount, int pop) {
        ...
        try {
            File file = new File(folderName + "/run" + run + "cheaters" + cheatCount + "of" + pop + ".dat");
            file.createNewFile();
            ...
            System.out.println("Writing failed with IOException.  Disabling saving for this simulation...");
            ex.printStackTrace();
            saveData = false;
        }
    }
    
    public void setSave(boolean b) { saveData = b; }
    public boolean isSaving() { return saveData; }
    
    public void setNormalize(boolean b) { normalizeIV = b; }
    public boolean isNormalizing() { return normalizeIV;}
    public void setNormalizedIV(double d) { normalizedIV = d; }
    public double getNormalizedIV() { return normalizedIV; }
