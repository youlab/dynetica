...
public class Ribosome extends GeneExpressionMachinery {
    ...
    public Ribosome(String name, GeneticSystem system, double elongationRate,
            double spacing, double iv, double min, double max) {
        setName(name);
        setSystem(system);
    ...
    public GeneralExpression getTotalTranslationActivity() {
        ...
        for (int i = 0; i < genes.size(); i++) {
            ...
            if (g.isTranslatable())
                ge = DMath.sum(ge,
                        DMath.multiply(g.getRna(), g.getKTranslation()));
        }
