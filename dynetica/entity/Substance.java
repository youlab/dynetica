...
public class Substance extends EntityVariable {
    ...
    //a rate expression to indicate the rate of change in the concentration of this substance
    GeneralExpression rateExpression;
    
    //Added on August 23 2006. This stores the "noise" term for each specices, following Lagevin formulation
    GeneralExpression stochasticTerm;
            

    /**
     * Substance class describes the entities with an emphasis on their quantities
    ...
    /**
     *sets the rate expression for the substance directly, only if the substance
     *doesn't participate in any reaction
     *
    ...
    public void setRateExpression(String exprStr) throws IllegalExpressionException{
	try {
	    if (rateExpression == null && !hasReactions()) {
		rateExpression = ExpressionParser.parse(getSystem(), exprStr);
	    }
	    else {
		System.out.println("WARNING: rate expression for: " + this + " has already been set.");
	    } 
	}
    ...
    //
    // set the rate expression for this substance based on the rate expressions of the reactions
    // that this substance is involved in. 
    //  
    // although this method appears messy, it doesn't. the underlying logic is quite simple.
    // 
    // all it does is to collect the rate expression from each reaction and its stoichiometry
    // from each *ProgressiveReaction* and combine them to the current rate expression. 
    //
	...
	while (rItr.hasNext()) {
	    ...
	    // it's impossible to construct rate expression for Equilibrated reactions.
	    if (c != 0.0) {
		if (r instanceof ProgressiveReaction) {
		    if (rateExpression == null) {
			if (c == 1.0)
			    rateExpression = ((ProgressiveReaction) r).getRateExpression();
			else
			    rateExpression = 
				ExpressionBuilder.buildBinary ("*", new Constant(c),
							       ((ProgressiveReaction) r).getRateExpression());
		    }
		    else {
			if (c == 1.0)
			    rateExpression = 
				ExpressionBuilder.buildBinary ("+", rateExpression, 
							       ((ProgressiveReaction) r).getRateExpression());
			else if (c == -1.0)
			    rateExpression = 
				ExpressionBuilder.buildBinary ("-", rateExpression, 
				...
				GeneralExpression ge = 
				    ExpressionBuilder.buildBinary("*", new Constant(c),
								  ((ProgressiveReaction) r).getRateExpression());
				rateExpression = ExpressionBuilder.buildBinary("+", rateExpression, ge);
			    }
			    else {
				GeneralExpression ge = 
				    ExpressionBuilder.buildBinary("*", new Constant(-c),
   ...
   public void setStochasticTerm() {
	...
	while (rItr.hasNext()) {
		...
		if (r instanceof ProgressiveReaction) {
			...
			if (c == 1.0)
			    stochasticTerm = ((ProgressiveReaction) r).getStochasticRateExpression();
			else
			    stochasticTerm = 
				ExpressionBuilder.buildBinary ("*", new Constant(c),
							       ((ProgressiveReaction) r).getStochasticRateExpression());
		    }
		    else {
			if (c == 1.0)
			    stochasticTerm = 
				ExpressionBuilder.buildBinary ("+", stochasticTerm, 
							       ((ProgressiveReaction) r).getStochasticRateExpression());
			else if (c == -1.0)
			    stochasticTerm = 
				ExpressionBuilder.buildBinary ("-", stochasticTerm, 
				...
				GeneralExpression ge = 
				    ExpressionBuilder.buildBinary("*", new Constant(c),
								  ((ProgressiveReaction) r).getStochasticRateExpression());
				stochasticTerm = ExpressionBuilder.buildBinary("+", stochasticTerm, ge);
			    }
			    else {
				GeneralExpression ge = 
				    ExpressionBuilder.buildBinary("*", new Constant(-c),
    ...
    public GeneralExpression getRateExpression() {
        ...
        // NOTE Lingchong You. The following statement is added to ensure that the rate expression
        // is updated. But it may slow down the program. This needs to be found out.
        //
        if (rateExpression == null) setRateExpression();
        //End Modified.
	...
	if (rateExpression == null) return 0.0; 
	return rateExpression.getValue();
    }
    
    public double getNoise() {
        if (stochasticTerm == null) return 0.0;
 ...
 //       System.out.println(value);
    ...
    public void storeValue() { values.add(value); }
    
    public double [] getValues() { return values.doubleValues();}
    ...
    public void setProperty(String propertyName,String propertyValue) throws UnknownPropertyException, InvalidPropertyValueException {
        ...
        // the following two if statements were added on 4/19/2005
           ...
           setY(Double.parseDouble(propertyValue));
            ...
            throw 
            new UnknownPropertyException("Unknown property for Substance " + 
