...
public class Genome extends SimpleSystem {
    ...
    /**
     * Genome class describes how the genetic elements are organized
     * in the genome. It is often initialized from an input file which
     * stores the information about a paticular genome. <p>
     */
...
//    boolean translocationSet = false;
    GenomeTranslocation translocation = null;
     ...
     /** Holds value of property lengthInCell.  This variable indicates the length of the part of 
     the ViralGenome that is inside the cell. It's useful in situations where the viral genome entry
     is sufficiently slow that it matters. One example is the entry of T7 genome into the host
     cell.*/
  ...
  //          System.out.println("Inserting " + ge.getName());
            ...
            //
            if (ge instanceof Gene) genes.add((Gene) ge);
            else if (ge instanceof Promoter) promoters.add((Promoter) ge);
            else if (ge instanceof Terminator) terminators.add((Terminator) ge);
            else if (ge instanceof RNASplicingSite) RNASplicingSites.add((RNASplicingSite) ge);
            if (getSystem()!= null) 
    ...
    /**
     * puts a genetic element in the right position in the elements list.
     * (this is called when the starting position (in bp) of the genetic element
     * has been changed.
     */
	...
	StringBuffer s = new 
        StringBuffer(getFullName() +  "{" + NEWLINE);
 ...
 */
    ...
    public int getEnd() {
        if (elements.isEmpty()) 
            ...
            return ((GeneticElement) elements.get(elements.size()-1)).getEnd();
    }

    public void remove(String name) {
        Entity e = (Entity) get(name);
        super.remove(name);
        if (getSystem()!=null) getSystem().remove(getName() + "." + name);
        elements.remove(e);
 ...
 */
     ...
     * @param lengthInCell New value of property lengthInCell.
 */
 ...
 */
     ...
     * @param translocation New value of property translocation.
 */
