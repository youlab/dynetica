...
public class SimpleSystem extends Entity implements AbstractSystem {
    ...
    public void removeSystemStructureChangeListener(SystemStructureChangeListener l) {
 ...
 // the fire method.
     ...
     public void fireSystemStateChange() {
         this.saved = false;
         EventListener[] listeners = listenerList.getListeners(SystemStateChangeListener.class);
         // Process the listeners last to first, notifying
         // those that are interested in this event
         for (int i = 0; i < listeners.length; i++) {
              if (stateChangeEvent == null)
                     stateChangeEvent = new SystemStateChangeEvent(this);
              ((SystemStateChangeListener) listeners[i]).systemStateChanged(stateChangeEvent);
         }
     ...
     public void fireSystemStructureChange() {
         this.saved = false;
         //
         // a structure change for a system implies a state change. but not the other way around
         //
         fireSystemStateChange();
         EventListener[] listeners = listenerList.getListeners(SystemStructureChangeListener.class);
         // Process the listeners last to first, notifying
         // those that are interested in this event
         for (int i = 0; i < listeners.length; i++) {
              if (structureChangeEvent == null)
                     structureChangeEvent = new SystemStructureChangeEvent(this);
              ((SystemStructureChangeListener) listeners[i]).systemStructureChanged(
                     (SystemStructureChangeEvent) structureChangeEvent
                );
         }
          if (getSystem() != null) getSystem().fireSystemStructureChange();        
     }
	...
	if (!contains(entity.getName())) {
	    entities.put(entity.getName(), entity);
           ...
           System.out.println("Error: " + entity.getName() + " is already defined.");
        }
    }
    ...
    // to the master list using a long name (Genome's name + "." + GeneticElemnt's name).
    // This way, I can later access such a GeneticElement using its long name while
 ...
 */
     ...
     * @param saved New value of property saved.
 */
