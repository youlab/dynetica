...
public class ModularSystem extends ReactiveSystem{
    ...
    @Override
        ...
        if (!contains(entity.getName())) {
          ...
          try{
              ...
              System.out.println("Addition of "+entity.getName()+" to modules failed");
             }
            
            if(entity.getSystem().equals(this)){
             try{
                insertEntityIntoTreeModel(entity);
             }catch(Exception treeexp){treeexp.printStackTrace();}
             }
  ...
  public boolean insertEntityIntoTreeModel(Entity entity) {
         ...
         if(entity instanceof AbstractModule){
             if (moduleNode == null) {
                 ...
                 treeModel.insertNodeInto(moduleNode, systemNode, Math.min(systemNode.getChildCount(), 4));
             }
             treeModel.insertNodeInto(new DefaultMutableTreeNode(entity.getName()), moduleNode, moduleNode.getChildCount());
             return true;
            ...
            if (nodeName.compareTo(entity.getName()) == 0) {
                ...
                for (int i = pathToRoot.length - 2; i > 0; i--) {
                    if (pathToRoot[i].getChildCount() == 0) {
                        ...
                        else if (moduleNode== pathToRoot[i]) moduleNode=null;
                    }
                    else
                        return true;
                    }
    ...
    public void addToModules(Entity entity)
    {
        ...
        if(!(entity instanceof AbstractModule) && !(entity instanceof NetworkLayout)&& !(entity instanceof Annotation) && !(modules.isEmpty())){
    ...
    public void remove(String name) {
       if(this.contains(name)){
        ...
        //Kanishk Asthana 12 July 2013: This is temporary will be fixed later
        if(entity.getSystem().equals(this))
	...
	str.append(NEWLINE);
        ...
        for(int i=0; i<modules.size();i++){
          ...
          for(int j=0;j<module.getSubstances().size();j++)
          {
           ...
           if (!(s instanceof ExpressionVariable) && s.getSystem().equals(module))
	...
	str.append(NEWLINE);
        ...
        for(int i=0; i<modules.size();i++){
          ...
          {
           str.append(NEWLINE);
           ExpressionVariable ev=(ExpressionVariable) module.getExpressions().get(j);
	...
	str.append(NEWLINE);
        ...
        for(int i=0; i<modules.size();i++){
          ...
          for(int j=0; j < module.getProgressiveReactions().size(); j++){
           Reaction reaction=(Reaction) module.getProgressiveReactions().get(j);
          ...
          for(int j=0; j< module.getEquilibratedReactions().size(); j++){
           Reaction reaction=(Reaction) module.getEquilibratedReactions().get(j);
            if(reaction.getSystem().equals(module))
             str.append(reaction).append(NEWLINE);
          }
 ...
 public void merge(File systemFile) 
 ...
 UnknownPropertyException, InvalidPropertyValueException {
  ...
  java.util.List conflictedEntities= ModuleMerger.findConflicts(this, tempSystem);
  
  String warningMessage= "The System you are trying to import has "+ conflictedEntities.size()+ 
  ...
  if(answer==JOptionPane.YES_OPTION || answer==JOptionPane.CLOSED_OPTION){
  String suffix;
  
  do{
  
  suffix=JOptionPane.showInputDialog(null, "Please enter a suffix for all conflicted Entities\n"
           + "Ensure that there are no curly brackets or spaces in the suffix"
           , "Enter Suffix", JOptionPane.QUESTION_MESSAGE);
  
  }while(suffix.isEmpty() || suffix.contains("{") || suffix.contains("}") || suffix.contains(" "));
  ...
  for(int i=0 ; i<conflictedEntities.size(); i++){
   ...
   if(this.contains(newName.toString()) || tempSystem.contains(newName.toString())){
    String suffix2=suffix;
    do{
        
     suffix2=JOptionPane.showInputDialog(null,"The current system or the new one already has an entity"
            +" with the same name as: "+((new StringBuilder(originalName)).append(suffix2)).toString()+
            "\nPlease enter a new suffix for this entity."+
            "\nMake sure there are no spaces or curly brackets in the suffix","Problem!",
            JOptionPane.QUESTION_MESSAGE);
        
    }while(suffix2.isEmpty()|| suffix2.contains("{") || suffix2.contains("}") || suffix2.contains(" ")
            || this.contains(((new StringBuilder(originalName)).append(suffix2)).toString())
            || tempSystem.contains(((new StringBuilder(originalName)).append(suffix2)).toString()));
   newName.delete(0, newName.length());
   newName.append(originalName).append(suffix2);
   }
   en.setName(newName.toString());
   
   System.out.println("The new name of the Entity "+originalName+" is: "+en.getName());
  }
  ...
  else if(answer==JOptionPane.NO_OPTION)
  {
   for (int i=0; i<conflictedEntities.size(); i++){
      ...
      String originalClassName= this.get(tempEntityName).getClass().getName();
      
      if(originalClassName.equals(tempClassName))
      {
       String conflictedEntityType=originalClassName.substring(originalClassName.lastIndexOf('.') + 1);
       String newName=null;
       String message1="This ModularSystem already contains a "+conflictedEntityType+" with the name: "+originalEntityName;
       
       String message2="\nPlease enter a new name for the imported "+conflictedEntityType+" : "+ originalEntityName+"\n"+
       ...
       if(response==JOptionPane.YES_OPTION || response==JOptionPane.CLOSED_OPTION)
       {
        ...
        newName=JOptionPane.showInputDialog(null, message2, "Rename", JOptionPane.QUESTION_MESSAGE);
       
        }while( newName.contains("{") || newName.contains("}") || newName.contains(" ")
               || newName.equals(originalEntityName) || this.contains(newName));
       
        if(!newName.isEmpty())
        tempEntityName=newName;
       }
       if(!originalEntityName.equals(tempEntityName))
       tempSystem.get(originalEntityName).setName(tempEntityName);
      ...
      {
       String conflictedEntityType=originalClassName.substring(originalClassName.lastIndexOf('.') + 1);
       String tempEntityType=tempClassName.substring(tempClassName.lastIndexOf('.')+ 1 );
       String newName=null;
       String message1="This ModularSystem already contains a "+conflictedEntityType+" with the name: "+originalEntityName;
       
       String message2="\nPlease enter a new name for the imported "+ tempEntityType+" : "+ originalEntityName+"\n"+
                      "Make sure there are no spaces or curly brackets in the new name";
        do{
           
        newName=JOptionPane.showInputDialog(null, message1+message2, "Rename", JOptionPane.QUESTION_MESSAGE);
...
public void addModule(File systemFile) throws
 ...
 ModularSystem tempSystem=new ModularSystem();
 LineNumberReader reader = new LineNumberReader(new FileReader(systemFile));
 String line;
 reader.mark(100000);
  ...
  if(!((system instanceof ModularSystem) || (system instanceof GeneticSystem)))
 {
  ...
  reader.reset();
  ModuleMerger.set((SimpleSystem)mod, reader.getLineNumber(), ModuleMerger.getPropertiesString(reader));
  java.util.List conflictedEntities= ModuleMerger.findConflicts(this, tempSystem);
  ...
  fireSystemStructureChange();
        ...
        JOptionPane.showMessageDialog(null, "System not compatible!", "Error", JOptionPane.ERROR_MESSAGE); 
       }
}
 ...
 public void rename(String oldName, String newName) 
    {
        ...
        if (!contains(newName.trim())) {
            ...
            System.out.println("Error: The system already has " + newName + ".");
        }
