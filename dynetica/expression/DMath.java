...
// Adjust the simplification on Min, Max function so that if all the parameters are
...
public class DMath{
   ...
   public static GeneralExpression multiply(GeneralExpression a, GeneralExpression b){
   ...
   public static GeneralExpression substract(GeneralExpression a, GeneralExpression b){
   ...
   public static GeneralExpression divide(GeneralExpression a, GeneralExpression b){
 ...
 public static GeneralExpression abs(GeneralExpression a){
   ...
   public static GeneralExpression  compare(GeneralExpression x, GeneralExpression y){
	...
	if(xx > yy)   return new Constant(1.0);

	else if(xx < yy)   return new Constant(-1.0);
   ...
   public static GeneralExpression step(GeneralExpression x, GeneralExpression y){

       if( (x instanceof Constant) && (y instanceof Constant)){

	   if(x.getValue() > y.getValue()) return new Constant(1.0);
   ...
   public static boolean isParaConstSub(GeneralExpression exp){
	...
	if(exp instanceof Constant) return true;

	else if (exp instanceof Parameter) return true;

	else if (exp instanceof Substance) return true;
   ...
   public static   List toArray_re(GeneralExpression exp){
	...
	if(isParaConstSub(exp)){
	    ...
	    data.add(exp);
            ...
            GeneralExpression[] tempVar = ((FunctionExpression) exp).getVariables();
