...
/**
 * Usage: Delay (Substance, GeneralExpression ge) returns the value of a
 * substance back in history with the specified timeDelay. For example, if the
 * current time is Tc, the function will return the value of the Substance at Tc
 * - ge. If Tc-ge <=0, the function returns the initialValue of the substance.
 * 
 * 
 */
...
public class Delay extends FunctionExpression {
    ...
    //
    // 1) determine the indices of the time points that embrace the time point
    // of interest.
    ...
    public void compute() {
        ...
        // in current implemention, I'm returning the initial value of the
        // substance if the time delay is too large.
        if (td >= currentTime) {
        ...
        // better to throw an exception in the future.
            ...
            System.out
                    .println("Warning: Trying to get future values (negative time delay). Using present value instead");
        }
            ...
            // basic interpolation
            int a = Math.max(
                    getLowerIndex(timer.getTimePoints(), currentTime - td), 0);
            ...
            value = substance.getValue(a)
                    + (currentTime - td - timer.getTimePoint(a))
                    / (timer.getTimePoint(a + 1) - timer.getTimePoint(a))
    ...
    //
    // uses binary search to find the index of an element immediate preceeding
    // the value "key".
    //
    ...
    public String toString() {
        return "delay(" + substance.toString() + ", " + timeDelay.toString()
                + ")";
    }
