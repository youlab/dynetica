...
public class Translation extends ProgressiveReaction {
    ...
    public void setup() {
        ...
        if (aminoAcid != null)
            addSubstance(aminoAcid, -1.0);
        addSubstance(gene.getRibosome(), 0.0);
        addSubstance(protein, 3.0 / (gene.getLength()));
        rateExpression = DMath
                .multiply(gene.getRibosome().getElongationRate(),
                        DMath.min(DMath.multiply(gene.getRibosome(), DMath
                                .divide(DMath.multiply(gene.getKTranslation(),
                                        gene.getRna()), gene.getRibosome()
                                        .getTotalTranslationActivity())),
                        //
                        ...
                        // requirement.
                        //
                                DMath.divide(DMath.multiply(
                                        new Constant(gene.getLength() / 3.0),
                                        gene.getRna()), gene.getRibosome()
                                        .getSpacing())));
        //
        ...
        if (aminoAcid != null)
            rateExpression = DMath.multiply(
                    rateExpression,
                    DMath.divide(aminoAcid,
                            DMath.sum(aminoAcid, new Constant(1e4))));
    ...
    public void addSubstance(Substance s, Double d) {
        ...
        if (s.getName().compareTo("AminoAcid") == 0)
            aminoAcid = s;
    }
